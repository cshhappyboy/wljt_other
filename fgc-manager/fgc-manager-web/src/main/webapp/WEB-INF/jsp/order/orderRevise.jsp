								...
								onChange:function(newValue, oldValue){
									...
									if(newValue && srow){
										if(srow.materialcode == newValue){
											var ninvoicenum = srow.ninvoicenum;
											var ninnum = srow.ninnum;
											var ninfonum = srow.ninfonum;
											if(ninvoicenum > 0 || ninnum > 0 || ninfonum > 0){
												var ed = orderBDataGrid.datagrid('getEditor', {
													index : rindex,
													field : 'materialcode'
												});
												if(ed){
													ed.target.textbox('disable', true);
												}
	...
	*/
	deleteRow = function(){
		if(parent.$.modalDialog.last_edit_index != undefined){
			...
			var row = orderBDataGrid.datagrid('getSelected');
			console.info(row);
			var ninvoicenum = row.ninvoicenum;
			var ninnum = row.ninnum;
			var ninfonum = row.ninfonum;
			if(ninvoicenum > 0 || ninnum > 0 || ninfonum > 0){
				return;
			}
