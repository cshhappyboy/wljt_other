...
<script type="text/javascript">
	$(function() {
		...
		$('#pid').combotree({
			url : '${pageContext.request.contextPath}/res/resTree',
			parentField : 'pid',
			lines : true,
			panelHeight : 'auto',
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/res/updateRes',
			onSubmit : function() {
		...
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<th>资源类型</th>
					<td><select name="restypeId" class="easyui-combobox"
						data-options="width:140,height:29,editable:false,panelHeight:'auto'">
							<c:forEach items="${resTypeList}" var="resourceType">
								<option value="${resourceType.id}"
									<c:if test="${resourceType.id == resource.restypeId}">selected="selected"</c:if>>${resourceType.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input name="ename" class="easyui-textbox"
						value="${resource.ename}"></td>
					<th>法文</th>
					<td><input name="fname" class="easyui-textbox"
						value="${resource.fname}"></td>
				</tr>
				<tr>
					<th>葡萄牙文</th>
					<td><input name="mname" class="easyui-textbox"
						value="${resource.mname}"></td>
					<th></th>
					<td></td>
				</tr>
				<tr>
