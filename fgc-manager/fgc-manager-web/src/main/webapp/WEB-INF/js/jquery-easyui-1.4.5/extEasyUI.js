		...
		var opts = $.extend({
			...
			onMove:function(left,top){
				if(top < 0){
					...
					$(this).dialog('move',{left:left,top:600});
				}
				if(left < -930){
					$(this).dialog('move',{left:-930,top:top});
				}else if(left > 1230 ){
					$(this).dialog('move',{left:1230,top:top});
				}
			},
			onClose : function() {
...
$.refDialog = function(options) {
	if ($.refDialog.handler == undefined) {// 避免重复弹出
		var opts = $.extend({
			title : '',
			width : 840,
			height : 680,
		...
		opts.modal = true;// 强制此dialog为模式化，无视传递过来的modal参数
		return $.refDialog.handler = $('<div/>').dialog(opts);
	}
};
